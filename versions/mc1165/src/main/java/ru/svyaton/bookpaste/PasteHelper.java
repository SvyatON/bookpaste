...
public final class PasteHelper {
    ...
    public static Text createStatusMessage(int pagesUsed, String remaining, PageFitter fitter) {
        ...
        if (remaining.isEmpty()) {
            return BookPasteTextCompat.literal(I18n.translate("text.bookpaste.done", pagesUsed,
                    translateCountWord("page", pagesUsed, languageCode)));
        }

        int booksNeeded = Math.max(1, estimateBooksNeeded(remaining, fitter));
        if (pagesUsed == 0) {
            return BookPasteTextCompat.literal(I18n.translate("text.bookpaste.nospace", booksNeeded,
                    translateCountWord("book", booksNeeded, languageCode)));
        }

        return BookPasteTextCompat.literal(I18n.translate("text.bookpaste.partial", remaining.length(), booksNeeded,
                translateCountWord("book", booksNeeded, languageCode)));
    }
    ...
    public static Text createSignStatusMessage(int linesUsed, String remaining, PageFitter fitter) {
        if (remaining == null || remaining.isEmpty()) {
            ...
            return BookPasteTextCompat.literal(I18n.translate("text.bookpaste.sign.done",
                    shownLines,
        ...
        if (linesUsed <= 0) {
            return BookPasteTextCompat.literal(I18n.translate("text.bookpaste.sign.nospace",
                    signsNeeded,
                    translateCountWord("sign", signsNeeded, getLanguageCode())));
        }

        return BookPasteTextCompat.literal(I18n.translate("text.bookpaste.sign.partial",
                remaining.length(),
                signsNeeded,
                translateCountWord("sign", signsNeeded, getLanguageCode())));
    }
    ...
    public static final class PageSlice {
        ...
        public PageSlice(String pageText, String remainingText) {
