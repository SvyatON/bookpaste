...
public final class BookPasteConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addButton(new ButtonWidget(buttonX, this.height / 2 - 20, buttonWidth, 20, this.getModeButtonText(), button -> {
            this.formatMode = this.formatMode.next();
            button.setMessage(this.getModeButtonText());
        }));

        this.keyBindingButton = this.addButton(new ButtonWidget(buttonX, this.height / 2 + 18, buttonWidth, 20, this.getKeyBindingButtonText(), button -> {
            this.waitingForKey = true;
            this.waitingForKeyTicks = 0;
            button.setMessage(BookPasteTextCompat.translatable("text.bookpaste.config.hotkey.listening"));
        }));

        this.addButton(new ButtonWidget(centerX - 130, this.height / 2 + 46, centerButtonWidth, 20,
        ...
        this.addButton(new ButtonWidget(centerX + 4, this.height / 2 + 46, centerButtonWidth, 20,
                BookPasteTextCompat.translatable("text.bookpaste.config.hotkey.controls"),
                button -> MinecraftClient.getInstance().openScreen(BookPasteHotkeys.createControlsScreen(MinecraftClient.getInstance(), this))));

        this.addButton(new ButtonWidget(centerX - 102, this.height - 52, 100, 20,
        ...
        this.addButton(new ButtonWidget(centerX + 2, this.height - 52, 100, 20,
